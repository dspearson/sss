...
**Success Criteria** (what must be TRUE):
  ...
  4. `rpm-build/build-rpm.sh`, macOS and musl build scripts still produce artefacts with the `hybrid` feature enabled on every host in the existing release matrix (Linux x86_64, Linux aarch64 on `arm64-builder`, macOS arm64 on `macos-builder`, musl static).
**Plans**: TBD
